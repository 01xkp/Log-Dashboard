...
- 必须使用 HTTPS。仅开发环境允许 <code>http://localhost[:port]</code> 或 <code>http://127.0.0.1[:port]</code>；其他 HTTP 地址会被前端拒绝，且不会发出请求。
...
- 当前页面没有 Bearer Token 配置、输入或注入能力，也不会在请求中主动设置 <code>Authorization</code>。不要把令牌写入环境变量、URL 或页面配置。
...
每个列表项必须有 <code>id</code>、<code>received_at</code>、<code>status</code>；其他字段可省略。<code>id</code> 应是安全的不可猜测引用，不能带存储路径。
...
详情响应前端验证并保留的白名单为 <code>id</code>、<code>source_filename</code>、<code>received_at</code>、<code>device_ref</code>、<code>bytes</code>、<code>status</code>、<code>available_actions</code>。<code>id</code> 必须与请求的日志 ID 完全一致，<code>source_filename</code> 只能是文件名，不能是本机或服务端路径。
...
crc32, current_operation, cycle, device_file_state, device_ref, device_suffix,
...
free_mb, gatt_cache_refresh_attempted, gatt_cache_refresh_result,
...
raw_packet_hex_omitted, reason, record_mode, record_status, record_type, received_bytes,
reconnect_paused_for_background, rejected_frames, remaining_ms, request_bytes,
